...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to PirateCoin server / RPC client
            std::string strUsage = _("PirateCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  PirateCoin [options]                     " + "\n" +
                  "  PirateCoin [options] <command> [params]  " + _("Send command to -server or PirateCoin") + "\n" +
                  "  PirateCoin [options] help                " + _("List commands") + "\n" +
                  "  PirateCoin [options] help <command>      " + _("Get help for a command") + "\n";
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -pid=<file>            " + _("Specify pid file (default: PirateCoin.pid)") + "\n" +
...
//    }
    ...
    // Make sure only a single PirateCoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  PirateCoin is probably already running."), GetDataDir().string().c_str()));
    ...
    // requested to kill PirateCoin-qt during the last operation. If so, exit.
